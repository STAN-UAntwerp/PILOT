...
def random_sample(a, k):
    return np.random.choice(a, size=k, replace=False)
...
def process_con(
    ...
    return loss, best_node, best_loss, best_feature, interval, lm_L
    ...
    for feature_id in random_sample(np.arange(1, n_features + 1), max_features_considered):
        ...
        if feature_id - 1 not in categorical:
            ...
            for p in range(possible_p.shape[0] - 1):
                ...
                if ~np.isnan(loss).all() and (best_node == "" or np.min(loss) < best_loss):
